      ...
      <aside
        className={
          isMobile
            ...
            : `bg-slate-900 text-white shrink-0 hidden md:flex flex-col h-screen sticky top-0 transition-all duration-300 ease-in-out border-r border-slate-800 ${isCollapsed ? "w-20" : "w-64"
            }`
        }
      >
            ...
            >
              {({ isActive }) => (
                <>
                  ...
                  />
                  <span
                    className={`whitespace-nowrap overflow-hidden transition-all duration-300 ${isCollapsed && !isMobile ? "w-0 opacity-0" : "w-auto opacity-100"
                      }`}
                  >
                    {item.name}
                  </span>
        ...
        <Congrats isCollapsed={isCollapsed} isMobile={isMobile} />

        {/* Perfil de Usuario (Footer) */}
        <div className="p-3 border-t border-slate-700/50 bg-slate-900/50">
          <div className={`flex items-center justify-between gap-2 ${isCollapsed && !isMobile ? "flex-col" : ""}`}>
            <div
              className={`flex items-center gap-3 p-1 rounded-lg transition-colors flex-1 min-w-0 ${isCollapsed && !isMobile ? "justify-center" : ""
                }`}
            ...
            >
              ...
              {(!isCollapsed || isMobile) && (
                <div className="flex-1 min-w-0 overflow-hidden">
                  ...
                  <p className="text-xs text-slate-500 truncate">{user?.role || "Personal"}</p>
                </div>
              )}
            </div>
