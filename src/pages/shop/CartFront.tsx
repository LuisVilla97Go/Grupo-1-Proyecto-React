...
export default function CartFront() {
  const { cart, registerSale, updateQuantity, removeFromCart, clearCart } =
    useStore();
  ...
  const [paymentMethod, setPaymentMethod] = useState<
    "Efectivo" | "Tarjeta" | "Yape/Plin" | "Contraentrega"
  >("Contraentrega");
          ...
          <div>
            ...
            {cart.length > 0 && (
              <p className="text-slate-500 mt-1 text-sm">
                Tienes {totalItems}{" "}
                {totalItems === 1 ? "artículo" : "artículos"} en tu bolsa listos
                para comprar.
              </p>
            )}
          </div>
        ...
        {cart.length === 0 ? (
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-12 sm:p-24 text-center max-w-3xl mx-auto">
            ...
            <p className="text-slate-500 mb-8 max-w-md mx-auto text-sm">
              ...
              Descubre las mejores ofertas en nuestro catálogo.
            </p>
            <button
              onClick={() => navigate("/")}
            ...
            <div className="w-full lg:w-2/3 bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
              ...
              <div className="divide-y divide-slate-100">
                {cart.map((item) => (
                  <div
                    key={item.productId}
                    className="p-6 flex flex-col sm:flex-row gap-6 group hover:bg-slate-50/50 transition"
                  >
                    {/* Imagen */}
                    ...
                    <div className="flex-1 flex flex-col justify-between">
                      <div className="flex justify-between gap-4">
                        <div>
                          <h3 className="font-bold text-slate-800 text-base leading-tight group-hover:text-rose-600 transition pr-4">
                            {item.name}
                          </h3>
                          <p className="text-xs text-slate-400 mt-1.5 uppercase font-medium">
              ...
              <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 sm:p-8">
                ...
                <div className="pt-6 border-t border-slate-100 mb-8">
                  <div className="flex justify-between items-end">
                    ...
                    <span className="font-black text-3xl text-rose-600 tracking-tight">
                ...
                <p className="text-center text-xs text-slate-400 mt-4 font-medium flex items-center justify-center gap-1.5">
                  ...
                  {paymentMethod === "Contraentrega" &&
                    "Pago seguro al momento de recibir el producto"}
                  {paymentMethod === "Tarjeta" &&
                    "Pago seguro encriptado de extremo a extremo"}
                  {paymentMethod === "Yape/Plin" &&
                    "Pago rápido y sin comisiones"}
                  {paymentMethod === "Efectivo" && "Pago seguro en efectivo"}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 animate-in fade-in duration-200">
          ...
          <div className="relative bg-white rounded-3xl max-w-md w-full p-8 shadow-2xl border border-slate-100 animate-in zoom-in-95 duration-200 z-10 text-center">
            ...
            <p className="text-sm text-slate-500 mb-6">
              Estás a punto de completar tu pedido por un total de{" "}
            ...
            <div className="bg-slate-50 rounded-2xl p-5 mb-6 border border-slate-100 text-left text-sm space-y-3">
              <div className="flex justify-between">
            ...
            {paymentMethod === "Contraentrega" && (
              <div className="mb-6 space-y-3 text-left">
                ...
                <input
                  ...
                  className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-rose-500/20 text-sm"
                />
                <input
                  type="text"
              ...
              <button
                onClick={() => {
                  if (paymentMethod === "Contraentrega") {
                    ...
                    if (!name || !address) {
                      toast.error(
                        "Por favor completa los datos de envío para la contraentrega",
                      );
                      return;
                    }
                    executeCheckout(name, address);
                  } else {
                    executeCheckout();
                  }
                }}
