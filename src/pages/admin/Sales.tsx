...
export default function Sales() {
  ...
  const categoryCounts = useMemo(() => {
    return publishedProducts.reduce(
      (acc, p) => {
  ...
  const ticketPromedio = useMemo(
    () => (sales.length > 0 ? totalRecaudado / sales.length : 0),
    [sales, totalRecaudado],
  );
  ...
  const addToCart = (product: Product) => {
    ...
    if (existingIndex > -1) {
      ...
      if (currentQty >= product.stock) {
        toast.warning(
          `No puedes agregar más. Límite de stock alcanzado (${product.stock} disponibles).`,
        );
        return;
      }
      ...
      .map((item) => {
        if (item.productId === productId) {
          ...
          if (newQty > item.stockLimit) {
            toast.warning(
              `Límite de stock alcanzado (${item.stockLimit} disponibles).`,
            );
            return item;
          }
  ...
  const handleCheckout = () => {
    if (cart.length === 0) {
      toast.error(
        "El carrito está vacío. Agrega productos para realizar una venta.",
      );
      return;
    }
    ...
    for (const item of cart) {
      ...
      if (!currentProduct || currentProduct.stock < item.quantity) {
        toast.error(
          `Inconsistencia: "${item.name}" ya no tiene stock suficiente (${currentProduct?.stock || 0} disponibles).`,
        );
        return;
      }
    }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 font-medium">
              Caja activa y emisión de comprobantes en tiempo real
            </p>
          </div>
        </div>
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-200 space-y-4 shadow-sm">
              ...
              <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-xs font-extrabold tracking-wide transition whitespace-nowrap ${
                ...
                {filteredProducts.map((product) => {
                  ...
                  return (
                    <div
                      ...
                      className={`bg-white rounded-2xl border p-4 flex flex-col justify-between transition-all duration-300 shadow-sm ${
                        isOutOfStock
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-100">
                        ...
                        <button
                          ...
                          className={`w-7 h-7 rounded-full transition-colors flex items-center justify-center text-white ${
              ...
              {cart.length === 0 ? (
                <div className="py-20 text-center space-y-3">
                  ...
                  <p className="text-xs text-slate-400 max-w-[200px] mx-auto">
                    Selecciona productos a la izquierda para armar la
                    transacción.
                  </p>
                </div>
              ) : (
                  ...
                  {cart.map((item) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        <button
                          onClick={() =>
                            removeFromCart(item.productId, item.name)
                          }
                  ...
                  {[
                    ...
                    return (
                      <button
                        ...
                        onClick={() =>
                          setPaymentMethod(
                            method.id as "Efectivo" | "Tarjeta" | "Yape/Plin",
                          )
                        }
              ...
              >
                {isSubmitting ? (
                  ...
                  <>
                    ...
                    <span>
                      Completar Venta ({currencySymbol} {total.toFixed(2)})
                    </span>
                  </>
                )}
              </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
              ...
              <p className="text-2xl font-black text-slate-800">
                {currencySymbol}{" "}
                {totalRecaudado.toLocaleString("es-PE", {
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
              ...
              <p className="text-2xl font-black text-emerald-600">
                {currencySymbol}{" "}
                {ticketPromedio.toLocaleString("es-PE", {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}
              </p>
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex flex-col gap-1">
                            ...
                            {sale.customerName && (
                              <div className="text-[10px] text-slate-500 flex flex-col pl-3 border-l-2 border-slate-200 ml-1">
                                <span className="font-bold text-slate-600">
                                  Cliente: {sale.customerName}
                                </span>
                                <span>Envío: {sale.deliveryAddress}</span>
                              </div>
                            )}
                          </div>
                        </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-100 animate-in zoom-in-95 duration-200 z-10">
            ...
            <p className="text-xs text-slate-500 mb-6 leading-relaxed">
              ...
              Esta acción se guardará físicamente y descontará el stock de
              inmediato.
            </p>
